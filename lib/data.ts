...
export function getAllCertifications() {
  return data.credentials.certifications;
}

export function getCertificationBySlug(slug: string) {
  return data.credentials.certifications.find((cert) => cert.slug === slug);
}

export function getRelatedCertifications(currentSlug: string, limit = 2) {
  return data.credentials.certifications
